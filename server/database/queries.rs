// 
use tokio_postgres::Row;
use chrono::{DateTime, Utc};

// Use local crate "database/db.rs", us Type "PgPool"
use crate::database::db::PgPool;
// Use local crate "models.rs", use Struct "Item"
use crate::models::{
    Item, 
    CreateItem, 
    UpdateItem, 
    DeleteItem,
    Workspace,
    CreateWorkspace,
    UpdateWorkspace,
    DeleteWorkspace,
    WorkspaceTools,
    CreateWorkspaceTool,
    UpdateWorkspaceTool,
    DeleteWorkspaceTool,
    Spreadsheets,
    CreateSpreadsheet,
    UpdateSpreadsheet,
    DeleteSpreadsheet,
};

// Helper to convert SystemTime to DateTime<Utc>
fn system_time_to_datetime(st: std::time::SystemTime) -> chrono::DateTime<chrono::Utc> {
    chrono::DateTime::<chrono::Utc>::from(st)
}

// Helper Function to map a Row to an Item
fn row_to_item(row: &Row) -> Item {
    Item {
        id: row.get("id"),
        name: row.get("name"),
        description: row.get("description"),
        created_at: system_time_to_datetime(row.get::<_, std::time::SystemTime>("created_at")),
    }
}

// Query Function: Get All Items
// Return Vector of Item Structs or standard error
pub async fn list_items(pool: &PgPool) -> Result<Vec<Item>, Box<dyn std::error::Error>> {
    // Gets a Database client from the pool asynchronously
    let client = pool.get().await?;
    // Asynchronously preparing a SQL statement
    // SQL statement to fetch all items ordered by creation date (newest first)
    let stmt = client.prepare("SELECT id, name, description, created_at FROM items ORDER BY created_at DESC").await?;
    // Excute prepared SQL statement
    // rows is a Vector of Database rows returned by Query
    // query() takes reference to statement (stmt) and &[] parameters
    // &[] is empty slice, meaning no parameters needed on a GET stmt (no $1 or $2 needed)
    let rows = client.query(&stmt, &[]).await?;
    // items contains a Vec<Item> containing all items in the database
    // rows.iter() iterates over each row in the returned result
    // .map(row_to_item) Applies Helper Function "row_to_item" to each row converting it to an Item Struct
    // .collect() Collects the mapped items into a Vector
    let items = rows.iter().map(row_to_item).collect();
    // Returns the Vector of Items as a Successful Result
    Ok(items)
}

// Query Function: Create Item
// 
pub async fn create_item(pool: &PgPool, new: &CreateItem) -> Result<Item, Box<dyn std::error::Error>> {
    // 
    let client = pool.get().await?;

    // Prepare POST request statement
    let stmt = client.prepare("INSERT INTO items (name, description) VALUES ($1, $2) RETURNING id, name, description, created_at").await?;
    // 
    let new_item = client.query(
        &stmt, 
        &[&new.name, &new.description]
    ).await?;

    // 
    Ok(row_to_item(&new_item[0]))
}

// Query Function: Update Item (PUT)
// 
pub async fn update_item(pool: &PgPool, new: &UpdateItem) -> Result<Item, Box<dyn std::error::Error>> {
    //
    let client = pool.get().await?;
    // 
    let stmt = client.prepare("UPDATE items SET name = $1, description = $2 WHERE id = $3 RETURNING id, name, description, created_at").await?;
    // 
    let updated_item = client.query_one(
        &stmt,
        &[&new.name, &new.description, &new.id]
    ).await?;
    // 
    Ok(row_to_item(&updated_item))
}

// Query Function: Delete Item (DELETE)
// 
pub async fn delete_item(pool: &PgPool, target: &DeleteItem) -> Result<u64, Box<dyn std::error::Error>> {
    let client = pool.get().await?;
    let stmt = client.prepare("DELETE FROM items WHERE id = $1").await?;
    let result = client.execute(&stmt, &[&target.id]).await?;
    Ok(result)      // Returns the number of rows deleted
}


// Query Functions for Workspaces
// 
// Helper Function to map a Row to an Item
fn row_to_workspace(row: &Row) -> Workspace {
    Workspace {
        id: row.get("id"),
        user_id: row.get("user_id"),
        name: row.get("name"),
        description: row.get("description"),
        owner: row.get("owner"),
        created_at: system_time_to_datetime(row.get::<_, std::time::SystemTime>("created_at")),
        updated_at: system_time_to_datetime(row.get::<_, std::time::SystemTime>("updated_at")),
    }
}

// Query Function: Get All Workspaces
// Return Vector of Workspace Structs or standard error
pub async fn list_workspaces(pool: &PgPool) -> Result<Vec<Workspace>, Box<dyn std::error::Error>> {
    // Gets a Database client from the pool asynchronously
    let client = match pool.get().await {
        Ok(c) => c,
        Err(e) => {
            eprintln!("Error getting DB client: {:?}", e);
            return Err(Box::new(e));
        }
    };
    // Asynchronously preparing a SQL statement
    // SQL statement to fetch all items ordered by creation date (newest first)
    let stmt = match client.prepare(
        "SELECT 
            id,
            user_id, 
            name, 
            description,
            owner, 
            created_at,
            updated_at 
        FROM workspaces 
        ORDER BY created_at DESC"
    ).await {
        Ok(s) => s,
        Err(e) => {
            eprintln!("Error preparing SQL Statement: {:?}", e);
            return Err(Box::new(e));
        }
    };
    // Excute prepared SQL statement
    // rows is a Vector of Database rows returned by Query
    // query() takes reference to statement (stmt) and &[] parameters
    // &[] is empty slice, meaning no parameters needed on a GET stmt (no $1 or $2 needed)
    let rows = match client.query(&stmt, &[]).await {
        Ok(r) => r,
        Err(e) => {
            eprintln!("Error executing query: {:?}", e);
            return Err(Box::new(e));
        }
    };
    // items contains a Vec<Item> containing all items in the database
    // rows.iter() iterates over each row in the returned result
    // .map(row_to_item) Applies Helper Function "row_to_item" to each row converting it to an Item Struct
    // .collect() Collects the mapped items into a Vector
    let workspaces: Vec<Workspace> = rows.iter().map(row_to_workspace).collect();
    // Returns the Vector of Items as a Successful Result
    Ok(workspaces)
}

// Query Function: Create Workspace
// 
pub async fn create_workspace(pool: &PgPool, new: &CreateWorkspace) -> Result<Workspace, Box<dyn std::error::Error>> {
    // 
    let client = pool.get().await?;

    // Prepare POST request statement
    let stmt = client.prepare(
        "INSERT INTO workspaces (name, description, owner) VALUES ($1, $2, $3) RETURNING id, name, description, owner, created_at, updated_at").await?;
    // 
    let new_workspace = client.query(
        &stmt, 
        &[&new.name, &new.description, &new.owner]
    ).await?;

    // 
    Ok(row_to_workspace(&new_workspace[0]))
}

// Query Function: Update Workspace (PUT)
// 
pub async fn update_workspace(pool: &PgPool, new: &UpdateWorkspace) -> Result<Workspace, Box<dyn std::error::Error>> {
    //
    let client = pool.get().await?;
    // 
    let stmt = client.prepare("UPDATE workspaces SET name = $1, description = $2, owner = $3 WHERE id = $4 RETURNING id, name, description, owner, created_at, updated_at").await?;
    // 
    let updated_workspace = client.query_one(
        &stmt,
        &[&new.name, &new.description, &new.owner, &new.id]
    ).await?;
    // 
    Ok(row_to_workspace(&updated_workspace))
}

// Query Function: Delete Workspace (DELETE)
// 
pub async fn delete_workspace(pool: &PgPool, target: &DeleteWorkspace) -> Result<u64, Box<dyn std::error::Error>> {
    let client = pool.get().await?;
    let stmt = client.prepare("DELETE FROM workspaces WHERE id = $1").await?;
    let result = client.execute(&stmt, &[&target.id]).await?;
    Ok(result)      // Returns the number of rows deleted
}
